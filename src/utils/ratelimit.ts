...
export class RateLimiter {
  ...
  check(ip: string, path: string): { limited: boolean; remaining: number; resetTime: number } {
    ...
    if (!windowData || now - windowData.timestamp > this.windowMs) {
      // Window expired or doesn't exist, create new one
    ...
    if (limited) {
      logger.debug(`Rate limit exceeded for ${key} (${windowData.count}/${this.maxRequests})`);
    }
  ...
  private cleanup(): void {
    ...
    if (deleted > 0) {
      logger.debug(`Rate limiter cleanup: removed ${deleted} expired windows`);
    }
  }
