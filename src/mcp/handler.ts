...
const toolSchemas = [
    // --- NEW Authentication Orchestration Method ---
    {
        ...
        inputSchema: {
            ...
            required: [] // No required params from client
        }
    },
...
export const setupMCPHandler = (kickService: KickService): void => {
    ...
    // --- NEW Authentication Orchestration Method --- Uses kickService.auth
    jsonRpcServer.addMethod('kickAuth.initiateLogin', (params) => initiateLoginHandler(kickService.auth, params as any));

    // --- MODIFIED Authentication Methods --- Uses kickService.auth
    jsonRpcServer.addMethod('getAccessToken', (params) => getAccessTokenHandler(kickService.auth, params as any));
    jsonRpcServer.addMethod('refreshAccessToken', (params) => refreshAccessTokenHandler(kickService.auth, params as any));
    jsonRpcServer.addMethod('validateToken', (params) => validateTokenHandler(kickService.auth, params as any));
    jsonRpcServer.addMethod('revokeToken', (params) => revokeTokenHandler(kickService.auth, params as any));
    ...
    const getStoredAccessToken = async (): Promise<string> => {
        try {
            ...
            logger.error('Error retrieving access token:', error);
            if (error instanceof JSONRPCErrorException) {
                throw error; // Re-throw specific JSON-RPC errors
            }
            // Provide a more generic error if it's not a JSONRPCErrorException
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            throw new JSONRPCErrorException('Failed to retrieve access token.', -32011, errorMessage);
        }
    };
