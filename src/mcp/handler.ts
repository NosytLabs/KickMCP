...
/**
 * Sets up the MCP handler with all registered methods
 * @param kickService The KickService instance to use for handling requests
 * @returns The configured JSON-RPC server
 */
export const setupMCPHandler = (kickService: KickService): JSONRPCServer => {
    ...
    // Register all method handlers from toolSchemas
    toolSchemas.forEach(schema => {
        // Skip auth methods that are already registered above
        if (['kickAuth.initiateLogin', 'getAccessToken', 'refreshAccessToken', 'validateToken', 'revokeToken'].includes(schema.name)) {
            return;
        }
        
        jsonRpcServer.addMethod(schema.name, async (params) => {
            try {
                // Delegate to the appropriate KickService method
                const methodName = schema.name.replace('kick/', '');
                if (kickService[methodName]) {
                    return await kickService[methodName](params);
                }
                throw new JSONRPCErrorException(`Method ${methodName} not implemented`, -32601);
            } catch (error) {
                logger.error(`Error executing ${schema.name}:`, error);
                throw error;
            }
        });
    });
...
export const setupMCPHandler = (kickService: KickService): JSONRPCServer => {
    ...
    const getStoredAccessToken = async (params?: any): Promise<string> => {
        // First check if params contains an access_token
        if (params && params.access_token) {
            logger.debug('Using access token provided in request parameters.');
    ...
    jsonRpcServer.addMethod('createWebhook', async (params) => {
        ...
        const accessToken = await getStoredAccessToken(params); // Assuming user token for now
