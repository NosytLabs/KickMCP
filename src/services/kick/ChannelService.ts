...
export class ChannelService extends BaseKickService {
   ...
   * @param params Request parameters including channel_id or slug.
   * @returns Channel information.
   */
  async getChannelInfo(params: { channel_id: string }): Promise<KickTypes.Channel> { // Adjust Type if needed
    // Kick API uses slugs in the path for public channel info typically
    const endpoint = `/channels/${params.channel_id}`; // Verify if ID or slug is expected
    logger.debug(`Fetching info for channel ${params.channel_id}`);
  ...
  // These need verification against the actual Kick API structure
    ...
    async getChannelHosts(params: { access_token: string; channel_id: string }): Promise<any> {
        ...
        return this.makeRequest<any>('GET', endpoint, undefined, {}, true, params.access_