...
export class KickService {
  ...
  constructor() {
    this.baseUrl = process.env.KICK_API_BASE_URL || 'https://api.kick.com/api/v1';
  ...
  private async makeRequest<T>(method: string, endpoint: string, params?: any, headers?: any): Promise<T> {
    try {
      ...
      // Add authorization if access_token is provided in params
  ...
  async getOAuthUrl(params: { client_id: string, redirect_uri: string, scope: string, state?: string }) {
    const { client_id, redirect_uri, scope, state = '' } = params;
    const url = `https://kick.com/oauth2/authorize?response_type=code&client_id=${client_id}&redirect_uri=${redirect_uri}&scope=${scope}&state=${state}`;
    return { url };
  }

  async getAccessToken(params: { client_id: string, client_secret: string, code: string, redirect_uri: string }) {
    return this.makeRequest('POST', '/oauth2/token', {
  ...
  async validateToken(params: { access_token: string }) {
    return this.makeRequest('GET', '/oauth2/token/info', {}, {
      'Authorization': `Bearer ${params.access_token}`
    });
  }

  async revokeToken(params: { access_token: string }) {
    return this.makeRequest('POST', '/oauth2/token/revoke', {
  ...
  async getChatMessages(params: { access_token: string, channel_id: string }) {
    return this.makeRequest('GET', `/channels/${params.channel_id}/chat/messages`, {}, {
      'Authorization': `Bearer ${params.access_token}`
    });
  }

  async sendChatMessage(params: { access_token: string, channel_id: string, message: string }) {
    return this.makeRequest('POST', `/channels/${params.channel_id}/chat/messages`, {
  ...
  async getChannelInfo(params: { access_token: string, channel_id: string }) {
    return this.makeRequest('GET', `/channels/${params.channel_id}`, {}, {
      'Authorization': `Bearer ${params.access_token}`
    });
  }
  ...
  async getStreamInfo(params: { access_token: string, channel_id: string }) {
    return this.makeRequest('GET', `/channels/${params.channel_id}/stream`, {}, {
      'Authorization': `Bearer ${params.access_token}`
    });
  }
